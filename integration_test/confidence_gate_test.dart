// Live check of the confidence gate, through the app's own code path.
//
// A scan must be charged only for a result the user is actually shown. This
// drives PhotoRecognitionService with two real images and watches the counter:
// the unconfident one must raise LowConfidenceException and leave the count
// untouched, the confident one must succeed and spend exactly one.
...
import 'package:salamat/services/photo_recognition_service.dart';
import 'package:salamat/services/supabase_service.dart';

const int _shotPort = 8787;

/// 64x64 flat grey JPEG. The model has nothing to recognise: confidence 0.
const String _blankJpegB64 =
    ...
    'HR0dHf/dAAQABP/aAAwDAQACEQMRAD8AKKKKACiiigAooooAKKKKAP/QKKKKACiiigAooooAKKKK'
    'AP/RKKKKACiiigAooooAKKKKAP/SKKKKACiiigAooooAKKKKAP/Z';

/// 96x96 JPEG of one of the app's food icons: clears the threshold.
const String _foodJpegB64 =
    '/9j/4AAQSkZJRgABAQAASABIAAD/4QBMRXhpZgAATU0AKgAAAAgAAYdpAAQAAAABAAAAGgAAAAAA'
...
Future<File> _tempJpeg(String b64, String name) async {
...
Future<int> _eventCount() async {
  final rows = await SupabaseService.client.from('scan_events').select('id');
...
void main() {
  ...
  testWidgets('confidence gate: only a shown result costs a scan',
    ...
    debugPrint('GATE|uid=${SupabaseService.currentUser?.id}');
    debugPrint('GATE|before events=${await _eventCount()} '
        'left=${container.read(subscriptionProvider).scansLeft}');
    await shot('gate_01_before');

    // ── unconfident ──
    final blank = await _tempJpeg(_blankJpegB64, 'blank');
    try {
      final r = await PhotoRecognitionService.recognizeFood(blank);
      debugPrint('GATE|unconfident returned $r  <-- expected a throw');
    } on LowConfidenceException catch (e) {
      debugPrint('GATE|unconfident threw LowConfidenceException '
          'confidence=${e.confidence}');
    } catch (e) {
      debugPrint('GATE|unconfident threw ${e.runtimeType}: $e');
    }
    await subs.refreshFromServer();
    await settle(tester, ms: 1200);
    debugPrint('GATE|after unconfident events=${await _eventCount()} '
        'left=${container.read(subscriptionProvider).scansLeft}');
    await shot('gate_02_after_unconfident');

    // ── confident ──
    final food = await _tempJpeg(_foodJpegB64, 'food');
    try {
      final r = await PhotoRecognitionService.recognizeFood(food);
      debugPrint('GATE|confident ok name=${r?['name']} '
          'confidence=${r?['confidence']} scan=${r?['_scan']}');
    } catch (e) {
      debugPrint('GATE|confident threw ${e.runtimeType}: $e');
    }
    await subs.refreshFromServer();
    await settle(tester, ms: 1200);
    debugPrint('GATE|after confident events=${await _eventCount()} '
        'left=${container.read(subscriptionProvider).scansLeft}');
    await shot('gate_03_after_confident');
  });
}
