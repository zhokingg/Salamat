// Live check that an exhausted allowance reaches the limit sheet, not an error.
//
// Burns the three free scans through the app's own service, then asks for a
// fourth and watches what comes back. The camera itself cannot run on the
// simulator, so the final hop (shutter -> sheet) is not executed here; what is
// verified is the input to that decision (QuotaExhaustedException rather than a
// generic failure) and both destinations it leads to.
...
/// 96x96 JPEG of a food icon: clears the confidence threshold, so each call
/// really does spend a scan.
...
void main() {
  ...
  for (final lang in const ['en', 'ru']) {
    testWidgets('exhausted quota reaches the limit sheet ($lang)',
      ...
      debugPrint('Q|$lang uid=${SupabaseService.currentUser?.id} '
          'left=${container.read(subscriptionProvider).scansLeft}');

      final f = File('${Directory.systemTemp.path}/q_food.jpg');
      await f.writeAsBytes(base64Decode(_foodJpegB64));

      // Burn the three free scans.
      for (var i = 1; i <= 3; i++) {
        try {
          final r = await PhotoRecognitionService.recognizeFood(f);
          debugPrint('Q|$lang burn$i ok scan=${r?['_scan']}');
        } catch (e) {
          debugPrint('Q|$lang burn$i threw ${e.runtimeType}');
        }
      }
      await subs.refreshFromServer();
      await settle(tester, ms: 1200);
      debugPrint('Q|$lang after burn '
          'left=${container.read(subscriptionProvider).scansLeft}');

      // The fourth attempt: what does the app actually get?
      try {
        final r = await PhotoRecognitionService.recognizeFood(f);
        debugPrint('Q|$lang fourth returned $r  <-- expected a throw');
      } on QuotaExhaustedException catch (e) {
        debugPrint('Q|$lang fourth threw QuotaExhaustedException '
            'used=${e.used} allowance=${e.allowance}');
      } catch (e) {
        debugPrint('Q|$lang fourth threw ${e.runtimeType}: $e');
      }

      // Destination 1: the camera screen with nothing left.
      appRouter.push('/camera');
      await settle(tester, ms: 3000);
      await shot('quota_${lang}_01_camera_out');
      appRouter.go('/dashboard');
      await settle(tester, ms: 1500);

      // Destination 2: the limit sheet the 402 branch opens.
      showPhotoLimitSheet(tester.element(find.byType(DashboardScreen)));
      await settle(tester, ms: 2000);
      await shot('quota_${lang}_02_limit_sheet');
    });
  }
}
